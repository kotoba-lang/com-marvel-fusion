(ns marvel-fusion.main
  "Kotodama WASM entrypoint for the marvel_fusion clean-room actor (L4) — Clojure port.
  ...
  facts (`marvel_fusion.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "marvel_fusion")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"marvel_fusion.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "marvel_fusion-compat" :tier tier :entities entities} 200])
...
(defn start! [] :marvel_fusion-compat/ready)
