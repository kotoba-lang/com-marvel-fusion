(ns marvel-fusion.main-test
  "Contract + behavioral test for the marvel_fusion-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "marvel_fusion-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'marvel-fusion.main-test)]
